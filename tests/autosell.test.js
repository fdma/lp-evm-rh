...
test('проценты превращаются в сотые доли процента, а не остаются процентами', () => {
  ...
  // Опечатка не подрезается, а отвергается: подрезанная, она молча
  // становится настройкой. Раньше 80 вместо 8.0 уходило с допуском 50%.
  reset({ slippage: 200 }); assert.strictEqual(A.slippageBps(), null, '200% — опечатка, отказ');
  reset({ slippage: 80 });  assert.strictEqual(A.slippageBps(), null, '80% — тоже');
  reset({ slippage: 20 });  assert.strictEqual(A.slippageBps(), 2000, 'ровно на потолке — можно');
});
...
// ── НАСТРОЙКИ СЕТЕЙ ─────────────────────────────────────────────────────────
//
// Этот тест появился после настоящей регрессии: при правке описания сетей у
// BNB Chain молча пропало поле kyberChain, и автопродажа на ней отвечала бы
// «сеть не поддержана». Ничего не падало — просто переставало работать на
// одной из двух сетей.
//
// Адрес роутера проверяется отдельно: ему выдаётся БЕЗЛИМИТНОЕ разрешение на
// монету, и пустое поле здесь означало бы, что сверять ответ агрегатора не с
// чем.
const C = require('../src/core.js');
test('у обеих сетей задан агрегатор и закреплён адрес его роутера', () => {
  for (const name of Object.keys(C.CHAINS)) {
    C.useChain(name);
    assert.ok(C.RH.kyberChain,
      `${C.RH.label}: не задан kyberChain — автопродажа скажет «сеть не поддержана»`);
    assert.match(C.RH.kyberRouter || '', /^0x[0-9a-f]{40}$/,
      `${C.RH.label}: не закреплён адрес роутера — сверять ответ будет не с чем`);
    assert.strictEqual(C.RH.kyberRouter, C.RH.kyberRouter.toLowerCase(),
      `${C.RH.label}: адрес роутера должен быть в нижнем регистре, он сравнивается строкой`);
  }
});
