...
const CHAINS = {
  robinhood: {
    ...
    // Имя сети у агрегатора KyberSwap. Он исполняет продажу: свой путь через
    // UniversalRouter собран и считает верно, но на ERC-20 парах свап
    // откатывается без объяснений — причина не найдена, подробности в шапке
    // autosell.js. Нативные свопы своим путём проходят.
    // АДРЕС РОУТЕРА АГРЕГАТОРА ЗАКРЕПЛЁН ЗДЕСЬ, А НЕ БЕРЁТСЯ ИЗ ОТВЕТА.
    //
    // Раньше он приходил из ответа агрегатора, и ему же выдавалось безлимитное
    // разрешение на монету. Это дыра: скомпрометированный или подменённый по
    // дороге сервис назвал бы чужой адрес, кошелёк подписал бы ему безлимит, и
    // монету вывели бы когда угодно позже — уже без участия страницы.
    //
    // Теперь адрес известен заранее, а ответ агрегатора с ним сверяется. Не
    // совпал — сделка не отправляется вовсе.
  ...
  bsc: {
    ...
    // Агрегатор, которым исполняется продажа, и его роутер — закреплён здесь
    // по той же причине, что и у Robinhood: ему выдаётся разрешение на монету,
    // и назначать этот адрес чужому серверу нельзя. Совпадает с адресом в
    // другой сети — проверено живым запросом к обеим.
    kyberChain: 'bsc',
    kyberRouter: '0x6131b5fae19ea4f9d964eac0408e4408b66337b5',
  },
};
...
//
// ВНИМАНИЕ, ПРЕЖДЕ ЧЕМ ЭТИМ ПОЛЬЗОВАТЬСЯ.
//
// Всё, что ниже до раздела «разрешения», СО СТРАНИЦЫ НЕ ВЫЗЫВАЕТСЯ. Продажу
// исполняет агрегатор (см. src/autosell.js). Этот код держат только тесты.
//
// Причина: на парах ERC-20 собранный здесь свап откатывается БЕЗ данных об
// ошибке, и причина не найдена. Исключено замерами на живой сети: кодирование
// (тем же кодировщиком свап нативной монеты проходит), ключи пулов, живость
// пула, баланс, разрешения, адрес роутера, проскальзывание, сумма, газ,
// направление. Трассировка на доступном тарифе узла закрыта.
//
// Считает он при этом ВЕРНО: на монете с 27 прямыми пулами дал 14.3334 USDG
// против 14.3336 у агрегатора, а на CHUMP нашёл путь через нативную монету в
// восемнадцать раз выгоднее прямого.
//
// Включать обратно можно только после сверки с НАСТОЯЩЕЙ успешной
// транзакцией — как сделано для входа и выхода. Структурные тесты этого не
// ловят: они разбирают собранное той же логикой, какой собирают.
//
// И ещё: planSwap внутри перебирает связки пулов ПОСЛЕДОВАТЕЛЬНО (до девяти
// котировок подряд, около 1.8 с). Перед включением это надо распараллелить —
// запросы независимы.
//
