...
(() => {
  ...
  // Строки таблицы позиций, готовые пересчитаться от новой цены, и пулы, на
  // которые мы ради этого подписаны. Живут ровно один проход loadPositions.
  let liveRows = [];
  ...
  // Снять подписку по ключу. Нужна там, где отписка происходит не в том месте,
  // где оформлялась: список позиций подписывается на пулы разом и так же разом
  // снимает те, где позиций больше нет.
  function wsUnsubscribe(key) {
    const s = ws.subs.get(key);
    if (!s) return;
    ws.subs.delete(key);
    wsDrop(s);
  ...
  //
  // Цена живая с самого начала, и СТОИМОСТЬ позиции теперь тоже: она приходит
  // подпиской на обмены пула и пересчитывается арифметикой, без похода в сеть
  // (см. liveRows ниже).
  //
  // Здесь остаётся то, за чем действительно надо идти к узлу: накопленные
  // комиссии и состав позиции. Раз в 20 секунд — чаще незачем, комиссии за это
  // время меняются на копейки, а каждый проход стоит запросов на позицию.
  //
  ...
  async function loadPositions() {
    ...
    tb.innerHTML = '';
    liveRows = [];          // строки прошлого прохода больше не наши
    ...
    for (const { id, liq } of live) {
      ...
      let cashOut = null, cashWhy = null, inRange = false;

      // ВЕСЬ РАСЧЁТ, ЗАВИСЯЩИЙ ОТ ЦЕНЫ, СОБРАН ЗДЕСЬ — И ВЫЗЫВАЕТСЯ ПОВТОРНО.
      //
      // Цена в терминале живая: она приходит подпиской в момент обмена. А
      // стоимость позиции до сих пор пересчитывалась только раз в двадцать
      // секунд, вместе с походом в сеть за комиссиями. Получалось, что сверху
      // цена движется, а в таблице стоит старая цифра.
      //
      // Ничего нового считать не надо: состав, стоимость, итог и «в работе /
      // ждёт» выводятся из цены чистой арифметикой. Поэтому расчёт собран в
      // функцию, которая ПЕРЕПРИСВАИВАЕТ те же переменные, и её достаточно
      // позвать заново с новой ценой — без единого запроса к узлу.
      //
      // pnlOf ниже замыкается на эти же переменные, поэтому подхватывает
      // новые значения сам.
      const recompute = (s0) => {
      comp = '—'; valueStr = '—'; total = null; stableSym = sym1; feesValue = 0;
      cashOut = null; cashWhy = null;
      inRange = Boolean(s0) && s0.tick >= t.tickLower && s0.tick < t.tickUpper;
      if (s0) {
      ...
      let rowRec = null;                    // запись о входе, когда найдётся
      const timeOf = (rec) => {
      ...
      (async () => {
        ...
        // Запись о входе нужна и потом: по ней пересчитывается итог, когда
        // цена меняется подпиской.
        rowRec = rec;
      ...
      liveRows.push({
        ...
        redraw: (s0) => {
          ...
          // ячейке стоит «ищу вход в цепочке…», и затирать её нечем.
          if (rowRec) tr.cells[5].innerHTML = pnlOf(rowRec);
          return true;
        },
      });
    }
    // ── ЦЕНА ПОЗИЦИЙ ТЕПЕРЬ ТОЖЕ ЖИВАЯ ──────────────────────────────────────
    //
    // Подписываемся на обмены КАЖДОГО пула, где есть позиция. Событие приносит
    // цену, а по ней строка пересчитывается арифметикой — ни одного запроса в
    // сеть. Раз в двадцать секунд остаётся только то, за чем действительно
    // надо идти к узлу: накопленные комиссии и состав позиции.
    //
    // Подписки снимаем по тем пулам, где позиций больше нет: иначе после
    // нескольких закрытий на узле копились бы потоки, которые никто не читает.
    {
      const want = new Set(liveRows.map(r => r.poolId));
      for (const pid of livePools) {
        if (!want.has(pid)) { wsUnsubscribe('pos:' + pid); livePools.delete(pid); }
      }
      for (const pid of want) {
        if (livePools.has(pid)) continue;       // уже подписаны
        livePools.add(pid);
        wsSubscribe('pos:' + pid,
          ['logs', { address: C.RH.poolManager, topics: [C.SWAP_TOPIC, pid] }],
          (l) => {
            if (!l.data) return;
            let s0;
            try {
              const w = C.words(l.data);
              // Раскладка та же, что у цены сверху: 2 — sqrtPriceX96, 4 — тик,
              // расширенный знаком до 256 бит.
              s0 = { sqrtPriceX96: BigInt('0x' + w[2]),
                     tick: Number(C.toSigned(BigInt('0x' + w[4]), 256)) };
            } catch (e) { return; }
            // Мёртвые строки (таблицу перерисовали) выбрасываем на месте.
            liveRows = liveRows.filter(r => r.poolId !== pid || r.redraw(s0));
          });
      }
    }
  ...
  async function settleClose(hash, tokenId, rec, symQuote, key, nativeBefore) {
    {
      ...
      if (rec && rec.amountIn != null && !entryUsable(rec, got)) {
        ...
        // КАРТОЧКУ ОТКЛАДЫВАЕМ ДО КОНЦА АВТОПРОДАЖИ.
        //
        // Раньше она показывалась здесь же, а после продажи показывалась
        // снова — уже с настоящим числом. Два полноэкранных окна подряд, и
        // первое из них с оценкой, которую второе тут же поправляет.
        //
  ...
  function drawAutosell() {
    ...
    const ready = Boolean(C.RH.kyberChain);
    const d = $('d-as'), s = $('s-as');
    if (!ready) {
      d.className = 'dot bad';
      s.textContent = `${C.RH.label}: сеть не поддержана`;
    } else if (S.on) {
      d.className = 'dot on';
      s.textContent = `готова, сеть ${C.RH.kyberChain}`;
    } else {
      d.className = 'dot';
  ...
  async function reportPnl({ d, ctx, saleReceipt, gasWei, nativeBefore }) {
    ...
    // Карточку отсюда НЕ показываем — её покажет autoSellAfter, и уже с этими
    // числами. Иначе окно выходило бы дважды.
    try {
      ledger.put(String(ctx.tokenId), { got: net, pnl, soldFor: fromSale, gasCost });
    } catch (e) { /* запись не критична */ }
  }

  // ИСПОЛНЕНИЕ.
  //
  // Порядок шагов выстрадан живой продажей, где сеть отклонила три попытки
  // подряд. Причина была не в цене: между расчётом и подписью проходило
  // двадцать секунд на два окна разрешений, и уходила протухшая котировка.
  //
  //   разрешение (один раз и навсегда) -> свежая котировка -> сразу отправка
  //
  ...
  async function autoSell(ctx) {
    ...
    try {
      ...
      if (!isNative(d.addr)) {
        ...
        if (have < d.raw) {
          // Без предела и один раз. Ограниченное «ровно на эту сумму» означало
          // бы новое окно перед КАЖДОЙ продажей — именно это и съедало время.
      ...
      // Цена пула, ИЗ КОТОРОГО вышли, — точка отсчёта для «во сколько обошёлся
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        ...
        log(`автопродажа: маршрут через ${plan.hops} ` +
            `${plan.hops === 1 ? 'пул' : 'пула'}${tail}. Дадут ` +
            `${outHuman == null ? plan.amountOut + ' (в базовых единицах)' : outHuman.toFixed(6)} ` +
            `${d.intoSym}` +
            (plan.amountInUsd ? ` ($${plan.amountInUsd.toFixed(2)} → $${plan.amountOutUsd.toFixed(2)})` : ''),
            'ok');

        if (homePrice && d.amount != null && outHuman != null) {
          const worth = (d.side === 0) ? d.amount * homePrice : d.amount / homePrice;
          if (worth > 0) {
        ...
        if (attempt === 3) {
          log('автопродажа: сеть отклонила три раза подряд. Монета в кошельке, ' +
              'продай вручную.', 'bad');
          return;
        }
